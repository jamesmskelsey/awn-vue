...
const REVIEW_DATE_REGEX = /((?:\d|\d\d)\/\d\d\/\d\d\d\d).*(?:\r|\n|\r\n).*(?:R.*V.*W(?:ED|D)*)/g;
// Looks for something that looks like a department heads name
// Had to match any number of ways they might sign their name
const REVIEWER_NAME_REGEX = /([A-Z]+,\s[A-Z]).*(?:\r|\n|\r\n).*(?:R.*V.*W(?:ED|D)?)/g;
  ...
  methods: {
    ...
    parseReport: (e, store) => {
      ...
      function reviewDate(str, wd) {
        ...
        var arr = [...str.matchAll(REVIEW_DATE_REGEX)];
      ...
      function lastReviewedBy(str) {
        var arr = [...str.matchAll(REVIEWER_NAME_REGEX)];
        if (arr.length == 0) {
          return "originator";
        } else {
...
<template>
  <div class="m-4">
    ...
    <input
      type="file"
      id="awnReport"
      v-on:change="handleAwnReport($data, $event)"
    />
