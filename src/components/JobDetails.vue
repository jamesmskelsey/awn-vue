  ...
  <div class="grid grid-cols-12">
    <div class="col-start-3 col-end-10">
      <h1 class="mt-4 text-2xl font-bold">
        {{ job.work_center }} - {{ job.jcn }}
      </h1>
      <h2 class="mt-3 text-xl font-semibold">
        {{ job.equipment }} in {{ job.location }}
      </h2>
      <hr />
      ...
      <div>
        ...
        <div class="m-2">
          ...
          <p class="text-gray-400 text-xs">
            This flag highlights that a job seems to mention that parts will be
            ordered:
          </p>
          <p
            :class="[
              job.should_have_parts && job.parts == 'NO' ? 'bg-red-300' : '',
              '',
            ]"
          >
            Has parts ordered: {{ job.parts }}
          </p>
        </div>
      </div>
      <hr />
      ...
      <div class="m-2">
        <p>
          Job is {{ job.days_old }} days old. It was reviewed on
          {{ job.actual_reviewed_date }} by {{ job.reviewed_by }}.
        </p>
        <hr />
